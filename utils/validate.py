...
def validate(model=None, data_loader=None, cfg=None, cls_loss_func=None):
    ...
    # Mean values for normalization, specific to the dataset
    ...
    with torch.no_grad():
        for data in tqdm(data_loader,
            ...
            is_simplified = getattr(model, "is_simplified", False)
            if is_simplified:
                cls1 = outputs[0]
                cls4 = outputs[0]
                cam_weights = None
            ...
            cls1 = merge_to_parent_predictions(
                cls1, k_list, method=cfg.train.merge_test)
            cls4 = merge_to_parent_predictions(
                cls4, k_list, method=cfg.train.merge_test)
            cls_loss = cls_loss_func(cls1, cls_label)
            cls4_acc_check = (torch.sigmoid(cls4) > 0.5).float()
            all_cls_acc4 = (cls4_acc_check == cls_label).all(
                dim=1).float().sum() / cls4_acc_check.shape[0] * 100
            avg_cls_acc4 = ((cls4_acc_check == cls_label).sum(
                dim=0) / cls4_acc_check.shape[0]).mean() * 100
            avg_meter.add({"all_cls_acc4": all_cls_acc4,
                          "avg_cls_acc4": avg_cls_acc4, "cls_loss": cls_loss})

            fused_cams = []
            for tta_trans in tta_transform:
                augmented_tensor = tta_trans.augment_image(inputs)
                tta_outputs = model(augmented_tensor)
                if getattr(model, "is_simplified", False):
                    cam2 = cam3 = cam4 = tta_outputs[1]
                    k_list = tta_outputs[9] if len(tta_outputs) > 9 else [
                        1] * cls1.shape[1]
                    cam_weights = None
                else:
                    cam2 = tta_outputs[3]
                    ...
                    cam_weights = tta_outputs[11] if len(
                        tta_outputs) > 11 else None

                cam2 = merge_subclass_cams_to_parent(
                    cam2, k_list, method=cfg.train.merge_test)
                cam3 = merge_subclass_cams_to_parent(
                    cam3, k_list, method=cfg.train.merge_test)
                cam4 = merge_subclass_cams_to_parent(
                    cam4, k_list, method=cfg.train.merge_test)
            ...
            img_denorm_tensor = torch.clamp(
                img_denorm_tensor * 255, 0, 255).byte()
            img_np = img_denorm_tensor.permute(0, 2, 3, 1).cpu().numpy()

            refined_mask, _ = crf_processor.process(
                probs=probs_np, images=img_np)

            fuse_label234 = torch.from_numpy(refined_mask).long().cuda()

            fuse234_matrix.update(labels.detach().clone(),
                                  fuse_label234.clone())

    all_cls_acc4, avg_cls_acc4, cls_loss = avg_meter.pop(
        'all_cls_acc4'), avg_meter.pop("avg_cls_acc4"), avg_meter.pop("cls_loss")
...
def generate_cam(model=None, data_loader=None, cfg=None, cls_loss_func=None):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader,
            ...
            cam1 = merge_subclass_cams_to_parent(
                cam1, k_list, method=cfg.train.merge_test)
            cam2 = merge_subclass_cams_to_parent(
                cam2, k_list, method=cfg.train.merge_test)
            cam3 = merge_subclass_cams_to_parent(
                cam3, k_list, method=cfg.train.merge_test)
            ...
            cam_max = torch.max(fuse234, dim=1, keepdim=True)[0]
            # Using your background estimation method
            bg_cam = (1 - cam_max) ** 10
            # Note: The paper uses 5 classes (4 + background)
            full_probs_tensor = torch.cat([fuse234, bg_cam], dim=1)
            ...
            img_denorm_tensor = torch.clamp(
                img_denorm_tensor * 255, 0, 255).byte()
            img_np = img_denorm_tensor.permute(
                0, 2, 3, 1).cpu().numpy()  # Shape: [B, H, W, C]
            ...
            for i in range(len(output_refined)):
                pred_mask = Image.fromarray(output_refined[i].cpu(
                ).squeeze().numpy().astype(np.uint8)).convert('P')
